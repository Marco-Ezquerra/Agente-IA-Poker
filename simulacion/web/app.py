...
class PartidaWeb:
    ...
    def _blueprint_query(self, bot):
        ...
        Clave del InfoSet: (player, street_idx, hand_bucket, bet_hist)
        El EHS² en hand_bucket ya codifica la textura del board implícitamente.
        ...
        try:
            ...
            # ── Bucket de la mano del bot en la calle actual (EHS²) ──────────
            ...
            n_buckets = PREFLOP_BUCKETS if self.street == 'preflop' else POSTFLOP_BUCKETS

            # ── Clave InfoSet sin board_buckets (reducción de espacio ×B³) ───
            key = (abstract_position, street_idx, hand_bucket, bet_hist)

            # ── Lookup con fallback progresivo ───────────────────────────────
            # 1. Historial exacto
            # 2. Historial vacío (inicio de calle, misma mano)
            # 3. Bucket vecino ±1 (suavizado por interpolación implícita)
            ss = _blueprint.strategy_sum.get(key)
            if ss is None or ss.sum() == 0:
                key_fb = (abstract_position, street_idx, hand_bucket, ())
                ss = _blueprint.strategy_sum.get(key_fb)
            if ss is None or ss.sum() == 0:
                for delta in (1, -1, 2, -2):
                    nb = hand_bucket + delta
                    if 0 <= nb < n_buckets:
                        key_nb = (abstract_position, street_idx, nb, ())
            ...
            # ── Samplear de la distribución (estrategia mixta GTO) ───────────
            avg    = ss / ss.sum()
